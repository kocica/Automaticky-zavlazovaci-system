...
void setup() {
  ...
  Wire.begin();
  /*
  for (int i = 0; i < 1000; i++)    // při mazání paměti se musí znovu nastavit obvod reálného času                                              // Vycisti EEPROM - 1kB
  EEPROM.write(i, 0);
  */
  int kontrolaEEPROM;
  for (int vycistiEEPROM = 0; vycistiEEPROM < 1000; vycistiEEPROM++) {
    kontrolaEEPROM = EEPROM.read(vycistiEEPROM);                                 // čtení z EEPROM
    if (kontrolaEEPROM == 255) {                                                 // Tam,kde je v EEPROM zapsano 256,to přepíše nulou
      EEPROM.write(vycistiEEPROM, 0);
    }
  }

  //zapisORC(0,58,9,1,20);
  //nastavení Obvodu Reálného Času, při deklaraci smazat poznámku (//) mimochodem čas lze nastavit i ručně, vpravo dole
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (i == 3) digitalWrite(rele[i], LOW);
  }

  //Tyto výstupy se přepnou do log. "1" pouze při čtení vstupů,pak se nastaví zpět na log. "0".
  pinMode(cteniVstupu1, OUTPUT);
...
void loop() {
  ...
  lcd.print("OUT:");
  lcd.print("1");
  if (((nastavvystupy1 == true) || (nastavvystupy2 == true)) || ((nastav_vystup[0] == 1) || (nastav_vystup[1] == 1) || (nastav_vystup[2] == 1) || (nastav_vystup[3] == 1) || (nastav_vystup[4] == 1) || (nastav_vystup[5] == 1) || (nastav_vystup[6] == 1)) || ((nastav_vystup[0] == 2) || (nastav_vystup[1] == 2) || (nastav_vystup[2] == 2) || (nastav_vystup[3] == 2) || (nastav_vystup[4] == 2) || (nastav_vystup[5] == 2) || (nastav_vystup[6] == 2))) {
    lcd.write(byte(1));
  ...
  lcd.print("2");
  if ((nastav_vystup[0] == 1) || (nastav_vystup[1] == 1) || (nastav_vystup[2] == 1) || (nastav_vystup[3] == 1) || (nastav_vystup[4] == 1) || (nastav_vystup[5] == 1) || (nastav_vystup[6] == 1) || (nastavvystupy1 == true)) {
    lcd.write(byte(1));
  ...
  lcd.print("3");
  if ((nastav_vystup[0] == 2) || (nastav_vystup[1] == 2) || (nastav_vystup[2] == 2) || (nastav_vystup[3] == 2) || (nastav_vystup[4] == 2) || (nastav_vystup[5] == 2) || (nastav_vystup[6] == 2) || (nastavvystupy2 == true)) {
    lcd.write(byte(1));
  ...
  for (i = 0; i <= 6; i++) {
    if ((hodiny == dny[i].hodinycelk) && (minuty == (dny[i].minutycelk - 1))) {
  ...
  if (!Binarnivstup1) {
    hlaseniPoruchy(nastavvystupy1, nastavvystupy2, Binarnivstup1, poc, poc2, i);    //zavolá fci hlášení poruchy, protože binární vstup 1 == 0
  }
  else {
...
int nastaveniParametru(int limitdisplaye, int nasthladiny, int nastvlhkosti) {
  ...
  for (;;) {
    ...
    if (stav == PRESSED && tlacitko != NO_KEY) {
      ...
      Serial.println(posledniZmackle);
  ...
  for (;;) {
    ...
    if (tlacitko) {
      ...
      if (tlacitko == 'E')
      {
        ...
        dny[i].min2celk = ((dny[i].min2 * 10) + dny[i].min20);
  ...
  for (;;) {
    ...
    if (tlacitko) {
      ...
      if (tlacitko == 'E')
      {
        ...
        if ((nastavORC[8] > 0) || (nastavORC[2] > 0)) {
