...
int soucasnyBehVystupy = 0;

unsigned long cekej, cekej01;     //časovače

int Menu = 0;              // Menu pro ethernet
int oba_vystupy, druhy_vystup;
int zhasnuti = 0, identifik = 0, kteryVystup = 2;
int nedele2 = 0;
...
int nastvlhkosti = 0, nasthladiny = 0, nasthystereze = 0;    // Přiřazení hodnot pro nastavování
...
void loop() {
  ...
  zobrazNaDisplayi();                                                                                 // Zobrazí na Displayi
  ...
  for (;;) {
    ...
    if (stav == PRESSED && tlacitko != NO_KEY) {
      ...
      if (posledniZmackle == 'E')
      {
        ...
        EEPROM.write(16, limitdisplaye);                        // zápis proměnných do eeprom!
        EEPROM.write(40, nasthystereze);
...
void problikavani() {
  if ((millis() > cekej01 + 1) && (millis() < cekej01 + 1500)) {
    lcd.setCursor(x, y);
    lcd.print(byte(3));               //blikání kurzoru
  }

  if ((millis() > cekej01 + 1500) && (millis() < cekej01 + 1600)) {
    lcd.setCursor(x, y);
    lcd.print(" ");                   //blikání kurzoru
  }
      ...
      if (c == '\n') {
        ...
        switch (Menu) {
          case 2:
            ...
            client.println("<br />");
            client.println("<h1>");
            client.print("Nastavit Limit Displaye = (max 999s)");
            client.println("</h1>");
            client.print(limitdisplaye);
            client.println("  -  ");
            client.println("<input type='number' style='background:yellow;width:38px' name='q8' min='0' max='999'>");
            client.println("<br />");
          ...
          case 5:
            ...
            if ((buffer[v] == 'q') && (buffer[v + 1] == '0') && (buffer[v + 2] == '=') && (buffer[v + 3] != '&') && (buffer[v + 4] == '&')) {   // nast minuty 1
