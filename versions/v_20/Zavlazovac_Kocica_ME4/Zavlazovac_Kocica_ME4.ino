...
bool nastavvystupy1 = false, nastavvystupy2 = false, nacitaniVstupu = true, soucasnyBeh = false, pozastavPoruchu = false,porucha = false, refresh[2] {false, false};
...
void loop()
{
  ...
  for (;;) {                                                                      // nekonečná smyčka (stejně jako void loop(), aby se nevytvářely pořád statické proměnné a objekt den
    ...
    if ((!Binarnivstup1) && (!pozastavPoruchu))
      den->hlaseniPoruchy(nastavvystupy1, nastavvystupy2, Binarnivstup1, i);      // zavolá fci hlášení poruchy, když (binární vstup 1 == NULL) a zároveň pozastav poruchu je neaktivni
