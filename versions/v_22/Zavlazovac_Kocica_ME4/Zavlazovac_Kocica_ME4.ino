...
void setup()
{
  ...
  */
  ...
  //EEPROM.write(10, 34);                         // slouží pro uložení druhýcg dvou cifer z hesla, které bude vyžadováno při startu a vypnutí podsvícení displaye, první číslo 9 neměnit, to je adresa v paměti, měnit druhé číslo!
  ...
  for (;;) {
    ...
    if ((stav == PRESSED && tlacitko != NO_KEY) || (irrecv.decode(&results))) {
      ...
      if ((posledniZmackle == '>') || (results.value == doprava))
      {
        ...
        if (a == 3) {
          if (limitdisplaye == 1) limitdisplaye = 2;
...
int Zapouzdreni_dat::hlaseniPoruchy(bool nastavvystupy1, bool nastavvystupy2, int Binarnivstup1, int i)
{
  ...
  for (;;) {
    ...
    if ((stav == PRESSED ) || (tlacitko)  || (irrecv.decode(&results))) {
      pozastavPoruchu = true;
      lcd.setCursor(17, 2);
      lcd.write("P=");
      lcd.setCursor(19, 2);
      lcd.write(byte(1));
      if ((tlacitko) || (results.value)) break;
    }
...
void Zapouzdreni_dat::nactiVstupy(int analogvstup1, int analogvstup2, int cteniVstupu1, int cteniVstupu2)
{
  ...
  AN1 = map(AN1, 161.5, kalibraceHladina, 0, 100);         // předdefinovaná fce map rozdělí 1. parametr v rozmezí od 2. parametru do 3. parametru na 0-100%.
