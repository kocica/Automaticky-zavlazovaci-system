...
int hlaseniPoruchy(bool nastavvystupy1, bool nastavvystupy2, int Binarnivstup1, int poc, int poc2, int i);                   //fce pro hlášení poruchy (binární vstup == NULL)
...
bez_znamenka long cekej, cekej01;                                                   //časovače

int identifik = NULL, kteryVystup = 2, nedele2 = NULL, oba_vystupy = NULL, druhy_vystup = NULL, BV1 = A8, soucasnyBehVystupy = NULL, AN1, AN2, Binarnivstup1, zhasnuti = NULL, Menu = NULL, rezim = NULL, rezimEEPROM = NULL;
...
void loop() {
  ...
  if (!Binarnivstup1) hlaseniPoruchy(nastavvystupy1, nastavvystupy2, Binarnivstup1, poc, poc2, i);    // zavolá fci hlášení poruchy, když binární vstup 1 == NULL
...
int Zapouzdreni_dat::cas(int limitdisplaye) {                //funkce cas
  lcd.backlight();
  int promenna = 1;
  lcd.setCursor(6, 2);
...
void Zapouzdreni_dat::zapisEEPROM() {
  ...
  if ((limitdisplaye >= NULL) && (limitdisplaye <= 250)) EEPROM.write(2 + rezimEEPROM, limitdisplaye);                        // zápis proměnných do eeprom!
...
void Zapouzdreni_dat::prectiEEPROM() {
  ...
  if (nasthladiny > 100) nasthladiny = NULL;

  nastvlhkosti = EEPROM.read(1 + rezimEEPROM);
  if (nastvlhkosti > 100) nastvlhkosti = NULL;

  nasthystereze = EEPROM.read(5 + rezimEEPROM);
  if (nasthystereze > 10) nasthystereze = NULL;

  limitdisplaye = EEPROM.read(2 + rezimEEPROM);
  if (limitdisplaye > 250) limitdisplaye = NULL;
  ...
  while (client.connected()) {
    ...
    if (client.available()) {
      ...
      if (c == '\n') {
        ...
        client.print("<a href='http://192.168.10.145?menu=13'><button style='background:orange;width:10%;height:80px'><font color='black'><h1>Režim</h1></font></button></a>");                    //režim
        client.print("<a href='http://192.168.10.145?menu=2'><button style='background:purple;width:90%;height:80px'><font color='silver'><h1>Nastavení parametrů</h1></font></button></a>");      //nastav parametru
        ...
        for (int v = NULL; v <= buffer.length(); v++) {
        ...
        switch (Menu) {
          ...
          case 13:
            ...
            client.println("<h1>");
            client.print("<font color='orange'>Nastavení režimu</font>");
            client.println("</h1>");
            client.println("<h3>");
            client.print("<font color='red'>Aktualní režim: </font>");
            ...
            switch (rezim){
              ...
              break;
              case 1:
                client.println("SUCHO");
              break;
              case 2:
                client.println("VLHKO");
              break;             
            }
            ...
            client.print("<P> <INPUT type='radio' name='rezim' value='0'>Režim NORMAL");
            client.print("<P> <INPUT type='radio' name='rezim' value='1'>Režim SUCHO");
            client.print("<P> <INPUT type='radio' name='rezim' value='2'>Režim VLHKO");
...
void Zapouzdreni_dat::prace_s_vystupy() {
  ...
  if (!soucasnyBeh) {
    for (i = NULL; i <= 6; i++) {
      ...
      if ((hodiny == dny[i].hodinycelk) && (minuty == dny[i].minutycelk) && (nastav_vystup[i] == NULL) && ((AN2 - nasthystereze) < nastvlhkosti) && (AN1 > (nasthladiny - nasthystereze)) && (denVTydnu == (i + 1)) && ((dny[0].min1celk > 0) || (dny[0].min2celk > 0))) {
        nastav_vystup[i] = 1;
        nacitaniVstupu = true;
...
Zapouzdreni_dat::Zapouzdreni_dat() {    //KONSTRUKTOR TŘÍDY
  prectiEEPROM();
}

//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

Zapouzdreni_dat::~Zapouzdreni_dat() {   //DESTRUKTOR TŘÍDY
