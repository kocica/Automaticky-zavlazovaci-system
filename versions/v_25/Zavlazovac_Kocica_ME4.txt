/*
  ...
  Program pro øízení zavlaovacího systému.
  ...
  pomocí tlaèítek nahoru, dolu, doleva a doprava na membránové klávesnici. Vıbìr a uloení dat se provedete tlaèítkem Enter a vıstup
  z funkce tlaèítkem Escape, nastavuje se pomocí èísel 0-9. Tlaèítka ‚F2‘, ‚#‘ a ‚*‘ se nevyuívají v ádnıch èástech programu.
  Pøi zmáèknutí ‚F1‘ kdekoli v programu se systém zablokuje. Pokud je u kteréhokoli vstupu/vıstupu plnı ètverec, znamená to,
  e je sepnutı (log. 1), pokud je u nìj prázdnı ètverec, znamená to, e je vypnutı (log. 0). Po kadém nastavení je nutno
  uloit data do EEPROM / ORC stisknutím tlaèítka Enter!

  Na prvním øádku LCD se nachází dny Pondìlí a Nedìle, kde mùete ve funkci „den“ nastavit kterı den, v jakou hodinu, minutu
  ...
  Na druhém øádku nastavujeme vlhkost (0-100%), hladinu (0-100%), hysterezi (0-10%), limit displaye (0-60 Minut) a reim
  ...
  a zároveò NH<MH (nastavená hladina bude menší ne hladina snímaná ultrazvukovım senzorem), bude mono vıstupy sepnout, v opaèném pøípadì
  pokud NV<MV nebo NH>MH vıstupy nebude mono sepnout a pokud u sepnuty byly, neprodlenì se rozepnou. Nastavení hystereze je ošetøení proti
  ...
  je mono provést pomocí libovolného tlaèítka. Vıbìr reimu usnadòuje práci se systémem, aby se nemusel pøenastavovat celı pøi náhlé zmìnì
  poèasí, ale staèilo jen pøepnout reim (Normal – Vlhko – Sucho).
  ...
  a nemoností manipulace se systémem. Pøi poruše nelze sepnout ádnı z vıstupù. Poruchu lze „pozastavit“ tím, e stlaèíte jakékoli tlaèítko
  na klávesnici, vypne se piezo bzuèák, blikání podsvícení displaye a bude monı pøístup ke všem funkcím programu, nebude ovšem mono v ádném
  ...
  Na ètvrtém øádku jsou zobrazeny vıstupy a aktuální den + èas. Vıstupy lze simulovat vıbìrem v menu sepnutím poadovaného vıstupu
...
#include <SPI.h>                                         //sériové periferní rozhraní, pouívá se pro komunikaci
#include <Ethernet.h>                                    //ethernetové rozhraní
#include "IRremote.h"                                    //obsahuje tøídy IRrecv a decode_results, které se pouívají pro práci s infraèervenım dálkovım ovladaèem

#define ORC_I2C_ADRESA 0x68                              //definice preprocesorovıch maker
#define NULL 0

// Piny pouité pro pøipojení LCD displaye (20x4, pøes i2c)
...
int receiver = A9;                 // pin 1 na IR pøijímaèi na Arduino digital pin A9

IRrecv irrecv(receiver);           // vytvoøí instanci 'irrecv'

decode_results results;            // vytvoøí instanci 'decode_results'
...
void zapisORC(byte sekundy, byte minuty, byte hodiny, byte denVTydnu, byte denVMesici);                                      //procedura s parametry pro uloení do obvodu reálného èasu
void pocitaniLitru(int pozice1, int pozice2, int pozice3, int pozice4, int pozice5);                                         //poèítá prùtok v m^3
void zpozdeni(int doba);                                                                                                     //procedura má za úkol zpozdit program na takovou dobu, jakı se jí dá parametr v ms(milisekundách)
void problikavani();                                                                                                         //procedura zajišující problikávání dvojteèky a šipky
...
void nastaveniNaEthernetu();                                                                                                 //pøi uloení na ethernetu se zobrazí na displayi
...
short x, y, i;                                                                  // x,y = kurzor, i = se pouívá ve všech cyklech for
...
bez_znamenka long cekej, cekej01, cekej02, litry = NULL, litryAktualni = NULL;                                             //èasovaèe + poèítání litrù (prùtok)
...
class Zapouzdreni_dat
{
  ...
  public:                                          // METODY / ÈLENSKÉ FCE TØÍDY
    ...
    int zadaniHesla(int adresaPameti, int zadejNeboZmen);                                                                        //slouí pro zadání hesla pøi startu a pøi zhasnutí podsvícení displaye
    bool vypnoutVystupy();                                                                                                       //pokud bude nedostatek hladiny nebo pøebytek vlhkosti, skoèí do hlavního menu a vypne vıstupy
    void resetKonfigurace();                                                                                                     //vymae všechna nastavení, tovární nastavení
    void litryZaMinutu(int pozice1, int pozice2);                                                                                //umonuje zadat prùtok èerpadla a zobrazuje v kubících kolik vody naèerpal
    ...
    void prace_s_vystupy();                                                                                                      //procedura, která zajišuje správnı bìh vıstupù (kdy se sepnou,na jak dlouho,kdy se rozepnou)
    void zobrazNaDisplayi();                                                                                                     //procedura má za úlohu vykreslit celé menu, se všemi hodnotami
    void stlaceniTlacitkaLoop();                                                                                                 //procedura, která zajišuje správnou fci tlaèítek
...
void setup()
{
  ...
  irrecv.enableIRIn();                          // start pøijímaèe
  ...
  */
  ...
  //EEPROM.write(9, 12);                          // slouí pro uloení prvních dvou cifer z hesla, které bude vyadováno pøi startu a vypnutí podsvícení displaye, první èíslo 9 nemìnit, to je adresa v pamìti, mìnit druhé èíslo!
  //EEPROM.write(10, 34);                         // slouí pro uloení druhıch dvou cifer z hesla, které bude vyadováno pøi startu a vypnutí podsvícení displaye, první èíslo 10 nemìnit, to je adresa v pamìti, mìnit druhé èíslo!
...
void loop()
{
  ...
  privitani(5000);                                                                // vypíše info o PRG a IP adr, pak pocka 3s bez monosti pøeskoèení

  //den->zadaniHesla(9, 1);                                                       // zavolá proceduru, ve které uivatel musí zadat správné heslo, jinak jej nevpustí do systému

  den->nactiVstupy(A14, A12, A15, A13);                                           // funkce která naète hodnoty z analog. a binar. vstupù a uloí je do globálních promìnnıch
  ...
  for (;;) {                                                                      // nekoneèná smyèka (stejnì jako void loop(), aby se nevytváøely poøád statické promìnné a objekt den
    ...
    pocitaniLitru(11, 12, 13, 14, 15);                                            // podle nastaveného prùtoku èerpadla pøipoèítává kadou vteøinu poèet naèerpané vody v kubících a ukládá do EEPROM
    ...
    if ((!Binarnivstup1) && (!pozastavPoruchu))                                   // zavolá fci hlášení poruchy, kdy (binární vstup 1 == NULL) a zároveò pozastav poruchu je neaktivni
    ...
    den->ethernet();                                                              // zajišuje síovou komunikaci
  }
...
int Zapouzdreni_dat::nastaveniParametru(int limitdisplaye, int nasthladiny, int nastvlhkosti, int nasthystereze)
{
  ...
  for (;;) {
    ...
    if ((millis() / 1000) > (cas2 + (60 * limitdisplaye))) {
      lcd.noBacklight();
      //zadaniHesla(9, 1);                                                            // zavolá proceduru, ve které uivatel musí zadat správné heslo, jinak jej nevpustí do systému
    }
  ...
  while (client.connected()) {
    ...
    if (client.available()) {
      ...
      if (c == '\n') {
        ...
        client.print("<a href='http://192.168.10.145?menu=14'><button style='background:black;width:100%;height:80px'><font color='gold'><h1>Zavlaovací systém Koèica Filip ME4</h1></font></button></a>");
        // client.println("<h1>");
        //client.println("Zavlaovací Systém, Koèica Filip ME4");
        // client.println("</h1>");
        ...
        client.print("<a href='http://192.168.10.145?menu=13'><button style='background:orange;width:20%;height:80px'><font color='black'><h1>Reim + Kalibrace</h1></font></button></a>");        //reim a kalibrace
        ...
        client.println("<title>* Ethernetové rozhraní pro zavlaovací systém, Koèica Filip ME4 *</title>");
        client.println("<meta http-equiv='refresh' content='60' >");   // refresh kadou minutu
        ...
        switch (Menu) {
          ...
          case 5:
            ...
            client.print("<P> <INPUT type='radio' name='denVTydnu' value='4'>Èt");
          ...
          case 13:
            ...
            client.print("<font color='green'>Nastavení Reimu + Kalibrace</font>");
...
void Zapouzdreni_dat::nactiVstupy(int analogvstup1, int analogvstup2, int cteniVstupu1, int cteniVstupu2)
{
  ...
  AN1 = map(AN1, 127.5, kalibraceHladina, 0, 100);         // pøeddefinovaná fce map rozdìlí 1. parametr v rozmezí od 2. parametru do 3. parametru na 0-100%.
...
void Zapouzdreni_dat::litryZaMinutu(int pozice1, int pozice2) {
  ...
  byte exponent_tri[8] = {     //plnı ètverec
  ...
  for (;;) {
    ...
    lcd.setCursor(0, 1);
    lcd.print((litryAktualni / 100000)); //pøevod cl -> m^3 (kubík)
    lcd.print(".");
    ...
    lcd.setCursor(0, 2);
    lcd.print((litry / 100000)); //pøevod cl -> m^3 (kubík)
    lcd.print(".");
...
bool Zapouzdreni_dat::vypnoutVystupy() {
  ...
  nactiVstupy(A14, A12, A15, A13);
  if (nastavvystupy2 == false && nastavvystupy1 == false) {       // pokud je vypnuta simulace vıstupù tak provìd podmínku !
    if ((AN2 > (nastvlhkosti + nasthystereze)) || ((AN1 + nasthystereze) < nasthladiny)) {
      for (i = NULL; i <= 6; i++) {
        if ((nastav_vystup[i] == 1) || (nastav_vystup[i] == 2) || (soucasnyBehVystupy == 1) || ((soucasnyBehVystupy == 2) && ((kteryVystup == NULL) || (kteryVystup == 1)))) {
          if (spustVystupy) minuty_osetreniVystupu = minuty + 1;
