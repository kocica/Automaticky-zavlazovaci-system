...
void loop()
{
  ...
  for (;;) {                                                                      // nekonečná smyčka (stejně jako void loop(), aby se nevytvářely pořád statické proměnné a objekt den
    ...
    if (!Binarnivstup1) den->hlaseniPoruchy(nastavvystupy1, nastavvystupy2, Binarnivstup1, i);    // zavolá fci hlášení poruchy, když binární vstup 1 == NULL
...
void privitani(int doba_trvani)
{
  ...
  lcd.print("*       v2.0       *");
  lcd.setCursor(0, 2);
  lcd.print("* KOCICA FILIP ME4 *");                                              // UVÍTÁNÍ, VYPSÁNÍ IP
  lcd.setCursor(0, 3);
  lcd.print("*IP: 192.168.10.145*");
  delay(doba_trvani);
  lcd.clear();
}
